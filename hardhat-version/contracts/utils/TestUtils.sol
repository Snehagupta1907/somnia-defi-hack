...
contract TestUtils  {
    ...
    function sqrtP60(uint256 price) public pure returns (uint160) {
        return TickMath.getSqrtRatioAtTick(tick60(price));
    }

    // Calculates sqrtP from tick with tick spacing equal to 60;
    function sqrtP60FromTick(int24 tick_) public pure returns (uint160) {
    ...
    function mintParams(
        ...
        uint256 amount0,
        uint256 amount1
    ) public pure returns (IUniswapV3Manager.MintParams memory params) {
    ...
    function mintParams(
        ...
        uint256 amount1,
        uint24 fee
    ) public view returns (IUniswapV3Manager.MintParams memory params) {
        params = IUniswapV3Manager.MintParams({
            tokenA: tokenA,
            tokenB: tokenB,
            fee: fee,
