...
const POOL_CONFIG = {
  name: 'Somnia USDTG-WSTT-STT Weighted Pool',
  symbol: 'SOMNIA-WP',
...
// Weights for the 3 tokens (must sum to 1e18)
const WEIGHTS = [
  ethers.parseUnits('0.4', 18),  // USDTG: 40%
  ethers.parseUnits('0.3', 18),  // WSTT: 30%
  ethers.parseUnits('0.3', 18)   // STT: 30%
];
...
async function main() {
  console.log('🚀 Step 1: Creating Somnia Weighted Pool...');
  ...
  // Note: Native STT (0x0) will be sorted first, then USDTG, then WSTT
  ...
  // For native tokens, we'll use a special approach
  const tokenConfig = sortedTokens.map((tokenAddress, index) => {
    const isNative = tokenAddress === ethers.ZeroAddress;
    
    console.log(`📋 Token ${index}: ${tokenAddress} (${isNative ? 'Native STT' : 'ERC20'})`);
  ...
  try {
    const tx = await factory.create(
      ...
      ethers.keccak256(ethers.toUtf8Bytes('somnia-weighted-pool-v1'))
    );
    ...
    fs.writeFileSync('scripts/somnia/deployment-weighted-pool.json', JSON.stringify(deploymentInfo, null, 2));
    console.log(`💾 Deployment info saved to scripts/somnia/deployment-weighted-pool.json`);
    ...
    console.log(`⚖️  Weights: USDTG(40%) WSTT(30%) STT(30%)`);
